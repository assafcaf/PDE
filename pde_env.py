...
class TwoPlayersPdeEnv(gym.Env):
    def __init__(self, window_size=2, table=prisoner, history_len=100, max_len=100) -> None:
        super().__init__()
        self.env_name = 'TwoPlayersPDE'
        self.window_size = window_size
        self.table = table
        self.table = np.resize(self.table, (2, 2, 2))
        self.history_len = history_len
    ...
    def step(self, actions: Any) -> tuple[Any, SupportsFloat, bool, bool, dict[str, Any]]:
        self.t += 1
        ...
        # apply action 
        transition  = np.array(actions)
        r1 = self.table[tuple(transition)][0]
        r2 = self.table[tuple(transition)][1]
           # save game history
        self.total_history.append(transition)
        self.history_deque.append(transition)
        ...
        return obs, np.array([r1, r2]), done, info
